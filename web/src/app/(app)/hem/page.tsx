"use client";

import { useEffect, useState } from "react";
import { useAuth } from "@/lib/auth";
import { api, ApiError, type CircleAccess } from "@/lib/api";
import { firstName } from "@/lib/labels";
import { CircleCard } from "@/components/CircleCard";
import { Spinner } from "@/components/Spinner";

export default function HemPage() {
  const { user } = useAuth();
  const [circles, setCircles] = useState<CircleAccess[] | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    let cancelled = false;
    async function load() {
      if (!user?.personId) {
        // Account not linked to a person → no memberships to derive circles from.
        setCircles([]);
        return;
      }
      try {
        const data = await api.personCircles(user.personId);
        if (!cancelled) setCircles(data);
      } catch (err) {
        if (!cancelled) {
          setError(
            err instanceof ApiError
              ? err.message
              : "Kunde inte hämta dina cirklar.",
          );
        }
      }
    }
    load();
    return () => {
      cancelled = true;
    };
  }, [user?.personId]);

  const greeting = firstName(user?.fullName ?? null)
    ? `Hej ${firstName(user?.fullName ?? null)}!`
    : "Hej!";

  return (
    <div className="animate-fade-in">
      <header className="mb-6">
        <p className="text-sm text-muted">Välkommen tillbaka</p>
        <h1 className="text-2xl font-semibold text-navy">{greeting}</h1>
      </header>

      <section>
        <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-muted">
          Mina cirklar
        </h2>

        {error ? (
          <p className="rounded-xl bg-red-50 px-4 py-3 text-sm text-red-700" role="alert">
            {error}
          </p>
        ) : circles === null ? (
          <div className="flex justify-center py-10">
            <Spinner className="h-6 w-6" />
          </div>
        ) : circles.length === 0 ? (
          <EmptyState linked={Boolean(user?.personId)} />
        ) : (
          <ul className="space-y-3">
            {circles.map((c) => (
              <li key={c.circleId}>
                <CircleCard circle={c} />
              </li>
            ))}
          </ul>
        )}
      </section>
    </div>
  );
}

function EmptyState({ linked }: { linked: boolean }) {
  return (
    <div className="rounded-2xl border border-dashed border-hairline bg-white/60 px-5 py-10 text-center">
      <p className="font-medium text-navy">
        {linked
          ? "Du är inte med i någon cirkel än"
          : "Ditt konto är inte kopplat till en person än"}
      </p>
      <p className="mt-1 text-sm text-muted">
        {linked
          ? "När du blir medlem i en cirkel dyker den upp här."
          : "En administratör behöver koppla ditt konto till en person."}
      </p>
    </div>
  );
}
